...
**v0.1 原件**：`archive/v0.1/DATA_MODEL.md`

## v0.2 调整

- **Node.prefab_ref.version**：从可选改为**必填**（强制版本锁）
- **Run 文件**：新增 `checkpoint.json`，支持断炵续跑
- **Event 类型**：新增 `run.paused` / `run.resumed`
- **节点 Schema**：改为 JSON Schema 单一来源（见 ARCHITECTURE §4）
- **存储**：文件写入带 file lock（见 ARCHITECTURE §10）
